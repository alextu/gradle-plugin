...
class BuildScanInjectionGradleIntegrationTest extends AbstractIntegrationTest {
  ...
  def 'build scan is not published without JENKINSGRADLEPLUGIN_BUILD_SCAN_INJECTION on manual step'() {
    given:
    gradleInstallationRule.gradleVersion = gradleVersion
    gradleInstallationRule.addInstallation()
...
""", false))
  ...
  def 'build scan is not published without JENKINSGRADLEPLUGIN_BUILD_SCAN_INJECTION on pipeline'() {
    given:
    gradleInstallationRule.gradleVersion = gradleVersion
    gradleInstallationRule.addInstallation()
    def pipelineJob = j.createProject(WorkflowJob)
...
""", false))
  ...
  def 'init script is copied in a custom gradle home'() {
    ...
    stage('Build') {
      node {
        withEnv(['JENKINSGRADLEPLUGIN_BUILD_SCAN_INJECTION=true','JENKINSGRADLEPLUGIN_GRADLE_ENTERPRISE_PLUGIN_VERSION=3.10.1','JENKINSGRADLEPLUGIN_CCUD_PLUGIN_VERSION=1.7','JENKINSGRADLEPLUGIN_GRADLE_ENTERPRISE_URL=http://foo.com','JENKINSGRADLEPLUGIN_BUILD_SCAN_OVERRIDE_HOME=/tmp']){
          withGradle {
            def gradleHome = tool name: '${gradleInstallationRule.gradleVersion}', type: 'gradle'
            writeFile file: 'settings.gradle', text: ''
            writeFile file: 'build.gradle', text: ""
...
""", false))
  ...
  def 'init script is deleted without JENKINSGRADLEPLUGIN_BUILD_SCAN_INJECTION'() {
    given:
    gradleInstallationRule.gradleVersion = gradleVersion
    gradleInstallationRule.addInstallation()
